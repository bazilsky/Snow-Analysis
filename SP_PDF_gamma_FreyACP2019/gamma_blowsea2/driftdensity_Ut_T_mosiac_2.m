...
%%estimate total in N
N_1 = DATA.N;
N_tot = nansum(DATA.N,2);

mu_1 = DATA.MU; % this is the drift density data
mu_nonnan = find(~isnan(mu_1))
mu_2 = mu_1(mu_nonnan)

t_1 = datetime(datestr(DATA.t));
t_mu = t_1(mu_nonnan);
...
% 
% for i = 1:(length(N_tot)-241)
%     flag = 1; % setting flag to filter BSn events 
% 
%     if N_tot(i) == 0 
%         N_tot_slice = N_tot(i+1:i+240);
%         sum_temp = nansum(N_tot_slice);
%         Bsn_temp = find(N_tot_slice~=0)
%         if length(Bsn_temp)>40
%             flag = 0;
%         end 
%         
%         if sum_temp == 0 & flag == 1
%             Bsn_count = Bsn_count +1 
%         end
%         
%     end
% end
...
% new code to count blowing snow events 
% take a 4 hourly rolling mean and check for zero values 
...
%while i < length(N_tot)

% % this is number of blowing snow events you get when you 
% 
i = 1
while (i+time_interval+1) < (length(N_tot)-1)
    if N_tot(i)==0 & N_tot(i+1)~=0
        N_tot_slice = N_tot(i+1:i+time_interval+1);
        zero_flag = find(N_tot_slice == 0);
        disp('length of zero flag = ')
        disp(length(zero_flag))
        if (length(zero_flag)<10) 
            BSn_count_2 = BSn_count_2+1;
            %disp('not a Bsn event')
        end
...
BSn_count_3 = 0;
% repeat the same above code to include drift density data instead to count
% blowing snow events 
i = 1
while (i+time_interval+1) < (length(mu_2)-1)

    if mu_2(i)==0 & mu_2(i+1)~=0
        mu_slice = mu_2(i+1:i+time_interval+1);
        zero_flag = find(mu_slice == 0);
        disp('length of zero flag = ')
        disp(length(zero_flag))
        if (length(zero_flag)<10)
            BSn_count_3 = BSn_count_3+1;
            disp('inside 2nd if ')
            %disp('not a Bsn event')
        end
...
lgd = legend({'U_{10m} vs T_{skin}'},'FontSize',14)
