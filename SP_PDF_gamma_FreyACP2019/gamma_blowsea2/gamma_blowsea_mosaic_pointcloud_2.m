...
t1 = datenum('01-Jan-2019 00:00'); 
t1_temp = t1;
t2 = datenum('01-Jul-2020 00:00'); % this statement is not read in 
...
while flag ==1
    ...
    t_mean = (t1+t1+1)/2 % bin mean 
    new_t_vector = [new_t_vector t_mean]
...
%   average if more than one line
    ...
    U8cm_mean = nanmean(DATA.U8cm_ex(n,:),1)
...
figure(3)
plot(new_t_vector,alpha, 'r*-','linewidth', 3)
hold on
plot(new_t_vector,beta, 'b*-','linewidth', 3)
set(gca, 'YScale', 'log')
...
figure(4)
plot(new_t_vector,alpha.*beta, 'r*-','linewidth', 3)
xlabel('Velocity bins (m/s)','fontsize',18)
...
%[X,Y] = meshgrid(U8cm_array,alpha.*beta);

N = 100; % Number Of Points Desired
d_mean = alpha.*beta;
U8cm_array(isnan(U8cm_array)) = 0;
d_mean(isnan(U8cm_array)) = 0;
num_points(isnan(U8cm_array)) = 0;

xv = linspace(min(U8cm_array), max(U8cm_array), N);
yv = linspace(min(d_mean), max(d_mean), N);
[X,Y] = ndgrid(xv, yv);

Z = griddata(U8cm_array, d_mean, num_points, X, Y);
figure(6)
contourf(X, Y, Z)
%axis('equal')
