...
delta = std(Ut);
figure(2) % this is plot for all 868 blowing snow events
...
T_fine_new = T_noaa_skin_2;

Ut_new = Ut0 + 0.0033.*(T_fine_new+27.27).^2;
delta_new = std(Ut);
Ut_min = Ut_new-2*delta_new;
Ut_max = Ut_new+2*delta_new;

Ut_filter = find(vel_zero > 4.5 & vel_zero < 9.5)

correct_vel_num = vel_zero(Ut_filter)
...
hold on
plot(T_fine,Ut+2*delta,'r--',T_fine,Ut-2*delta,'r--')
plot(mean_Tbin,mean_vel_zero,'b.','MarkerSize',12)
%errorbar([mean_Tbin,mean_Tbin], [vel25,vel75],'b')
errorbar(mean_Tbin, mean_vel_zero, vel25, vel75, 'b','LineWidth',1)
