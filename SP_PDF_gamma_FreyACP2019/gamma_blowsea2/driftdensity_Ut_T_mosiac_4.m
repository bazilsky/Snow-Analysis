...
%% old code to count blowing snow events 
% for i = 1:(length(x)-1)
%     if x(i) ~= (x(i+1)-1)
...
while (i+time_interval+1) < (length(mu_2)-1)
    ...
    if mu_2(i)==0 & mu_2(i+1)~=0
        ...
        if (length(zero_flag)<10)
            ...
            zeros_index = [zeros_index mu_2(i+1)]
            time_zero = [time_zero t_mu(i+1)]
            
            if i>10
                calc_vel_mean = nanmean(U2_mu(i-9:i+10))
            else
                calc_vel_mean = nanmean(U2_mu(i+1:i+7))
            end
            vel_zero  = [vel_zero U2_mu(i+1)]
            %vel_zero  = [vel_zero calc_vel_mean]
            disp('inside 2nd if ')
...
t_noaa_3 = datetime(datestr(DATA2.tower_NOAA.t)) % assigning the time noaa to this new variable. 
...
for i = 1:length(time_zero)
    ...
    T_noaa_skin_2 = [T_noaa_skin_2 T_noaa_skin(qw)]; % finding the corresponding temperature at that time. 
