...
N_temp = nansum(CLASP.conc(n0,1:16),2);
del_temp = 0;
...
for i = 1:length(N_temp)
    if i == 1
        del_temp = abs(N_temp(i+1)-N_temp(i));
    elseif i == length(N_temp)
        del_temp = abs(N_temp(i)-N_temp(i-1));
    else
        del_temp = abs(N_temp(i+1)-N_temp(i-1))/2;
    end

    N_del = [N_del; del_temp];
...
pda1 = find(N_del<=y_pda);
pda2 = find(N_del>y_pda);

figure(1)
plot(N_temp,y_pda,'k-','LineWidth',3)
hold on
plot(N_temp(pda1), N_del(pda1),'b.','MarkerSize',10)
hold on 
plot(N_temp(pda2), N_del(pda2),'r.','MarkerSize',10)
set(gca,'YScale','log')
set(gca,'XScale','log')
xlabel('N (cm^{-3})', 'FontSize', 18)
ylabel('dN/dt (cm^{-3} s^{-1})','FontSize', 18)
title('Filtering out local pollution', 'FontSize', 20)
set(gca,'FontSize',16)
