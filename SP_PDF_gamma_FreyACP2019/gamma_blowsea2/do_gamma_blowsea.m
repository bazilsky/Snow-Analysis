...
% compute PDF only for times when snow particles are detected
N_sum = nansum(N,2);
o = find(N_sum==0);
N(o,:) = [];
N_sum(o) = [];
...
for i=1:r
    ...
    data.y = dummy;
    p_init = [2.1 50];         % 1st guess for search [shape scale];
...
OUT = [t p_ret ep_ret exitflag matrix dp_mean ust U10m N_sum];
%% filter output
