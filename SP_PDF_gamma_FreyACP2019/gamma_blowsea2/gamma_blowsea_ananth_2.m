...
for i = 1:(length(velocity_bins)-1)
    ...
    n = find(DATA.U10m(:,1)>=velocity_bins(i) & DATA.U10m(:,1)<velocity_bins(i+1));
...
xlabel('Velocity_bins (m/s)','fontsize',14);
ylabel('No: of Data points', 'fontsize', 14)
title('Number of data points per velocity bin')
%legend('0-2','asdasd')
%legend (legend_lable')

figure(3)
plot(velocity_bins(1:(length(velocity_bins)-1)),alpha, 'r*-','linewidth', 3)
xlabel('Velocity_bins (m/s)','fontsize',18)
ylabel('alpha', 'fontsize', 18)
title('Shape parameter vs windspeed', 'fontsize', 20)
