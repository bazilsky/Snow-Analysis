...
velocity_bins = [0,2,4,6]
velocity_bins = (4:0.2:6)
alpha = []
beta  = []
legend_lable = []
str_temp = []
...
for i = 1:(length(velocity_bins)-1)
    ...
    n = find(DATA.U20cm(:,1)>=velocity_bins(i) & DATA.U20cm(:,1)<velocity_bins(i+1));
...
% average if more than one line
    ...
    str = sprintf('\\alpha %5.2f \\pm %0.2f, \\beta %5.2f \\pm %0.2f, U = %2.1f-%2.1f m/s',...
...
xlabel('Velocity bins (m/s)','fontsize',18)
ylabel('alpha*beta', 'fontsize', 18)
title('Mean diameter vs windspeed', 'fontsize', 20)
